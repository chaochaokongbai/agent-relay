...
## 它特殊在哪：**给 AI agent 的 branch protection**

别的工具管「谁干什么」（任务板 / 编排 / 记忆），接力棒管**「这份产物允不允许进工作树」**——
**编排器可以换，裁判不必换。**

| 同类在做 | 它们不做的事 |
|---|---|
| beads / Backlog.md：issue 与项目记忆（Markdown 进 git） | ✗ 不判断产物允不允许落地 |
| batty / vibe-kanban：编排、看板、git worktree、UI | ✗ 不产出可被第三方复验的裁决 |
| AGENTS.md / CLAUDE.md：给模型的说明书 | ✗ 没有状态机、没有门 |
| mem0 / Letta / MCP memory：长期记忆服务 | ✗ 与「代码准入」无关 |

接力棒只做三件别人没做全的事：

1. **能力分级准入**：`L0` 纯聊天客户端（粘贴通道，豁免工具调用计数）/ `L1` 有文件工具 / `L2` 声明可执行命令。
   **没有文件能力的客户端也能合法参与协作**，而不是被门一刀切拒绝。
2. **可复验裁决**：`relay verify|auto --json` 产出 `gate.version`（哪一版门放行的）+ 回执/变更/测试输出哈希
   + `chain{prev,self}` 哈希链；`relay recheck` 让**不信任本工具的人**也能自己验链、查漂移、重跑证据。
3. **零依赖、可嵌入**：门就是 `lib/gate.mjs`（无副作用），`import { executeVerify } from 'agent-relay/gate'`
   直接嵌进任何编排器 / CI —— **不做编排器，做编排器都要装的裁判**。

> 一句话：**任何模型都能交产物，只有过了门、留了证据的才准落地。**
